...
import {
  ...
  type LocalDeliveryPlanSettingsResponse,
...
const normalizeSolutions = (
  entry?: LocalDeliveryPlanSettingsResponse['route_solution'],
): RouteSolution[] => {
  if (!entry) return []
...
const normalizeStops = (
  entry?: LocalDeliveryPlanSettingsResponse['route_solution_stops'],
): RouteSolutionStop[] => {
  if (!entry) return []
...
const applyResponsePayload = (payload?: LocalDeliveryPlanSettingsResponse | null) => {
  if (!payload) return
