...
import {
  getPlanEndDateByLocalDeliveryPlanId,
...
const applyOptimizationPayload = (payload?: RouteOptimizationResponse | null) => {
  if (!payload) return
  normalizeSolutionMap(payload).forEach((solution) => {
  ...
  const stops = [
    ...normalizeStopsMap(payload.route_solution_stop),
    ...normalizeStopsMap(payload.route_solution_stop_skipped),
  ]
  stops.forEach((stop) => {
    if (stop?.client_id) {
